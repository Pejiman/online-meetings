Feature: signup
  ...
  Scenario Outline: signup without Enter less than 2 characters in the name field

   Given کاربر در صفحه ثبت نام قرار دارد
   When کاربر تمام فیلد ها را به جز فیلد نام تکمیل می کند
   And کاربر فیلد نام را با 1 کاراکتر پر می کند
   And کاربر روی دکمه ثبت‌نام کلیک می‌کند
   Then پیام نام می بایست حداقل 2 کاراکتر باشد نمایش داده می‌شود
    
    
  
Scenario Outline: signup without Enter more than 60 characters in the last name field

   Given کاربر در صفحه ثبت نام قرار دارد
   When کاربر تمام فیلد ها را به جز فیلد نام خانوادگی تکمیل می کند
   And کاربر فیلد نام خانوادگی را با بیش از 60 کاراکتر پر می کند
   And کاربر روی دکمه ثبت‌نام کلیک می‌کند
...
Scenario Outline: signup without Duplicate national code
...
Scenario Outline: signup without Birth date in wrong format
